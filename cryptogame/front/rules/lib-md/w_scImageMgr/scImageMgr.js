 ...
 *
 * Portions created by the Initial Developer are Copyright (C) 2009-2019
	...
	for(var i=0; i<this.fPathSlider.length; i++) {
		...
		for(var j=0; j<vSliders.length; j++) {
			...
			try {
				...
				vSlider.fImgs = scPaLib.findNodes("chi:img", vSlider);
				for (var k=0; k<vSlider.fImgs.length; k++) {
					var vImg = vSlider.fImgs[k];
					vImg.style.visibility = "hidden";
					vImg.onload = function() {
						this.fOrigWidth = this.width * (vSlider.fOrigHeight / this.height);
						this.style.position = "absolute";
						if (vSlider.fSliderWidth < this.fOrigWidth) {
							this.fWidth = vSlider.fSliderWidth;
							this.style.width = this.fWidth + "px";
							this.style.height = "auto";
							vSlider.fSliderHeight = scImageMgr.xGetEltHeight(this)
							vSlider.style.height = vSlider.fSliderHeight + "px";
						} else {
							this.fWidth = this.width * (vSlider.fSliderHeight / this.height);
							this.style.height = vSlider.fSliderHeight + "px";
							this.style.width = "auto";
						}
						vSlider.fToolsOver.style.height = vSlider.fSliderHeight + "px";
						this.style.left = vSlider.fSliderWidth + "px";
						if (this === vSlider.fImgs[0]) scImageMgr.xRunSlider(vSlider);
					}					
				}				
				vSlider.fToolsOver = scDynUiMgr.addElement("div", vSlider, vSlider.fOpts.clsPre+"ToolsOver");
				vSlider.fToolsOver.style.position = "absolute";
				vSlider.fToolsOver.style.top = "0";
				vSlider.fPauseBtn = scImageMgr.xAddBtn(vSlider.fToolsOver,vSlider,this.fTypSlider,"BtnPause",this.fStrings[18],this.fStrings[19]);
				vSlider.fPauseBtn.onclick = function(){return scImageMgr.xBtnMgr(this);}
				vSlider.fPauseBtn.fObj = vSlider;
				vSlider.fPlayBtn = scImageMgr.xAddBtn(vSlider.fToolsOver,vSlider,this.fTypSlider,"BtnPlay",this.fStrings[16],this.fStrings[17]);
				...
				var vResizer = {
					...
					onResizedAnc : function(pOwnerNode, pEvent) {
						...
						if (pOwnerNode.fSliderWidth < vCurrentImage.fOrigWidth) {
							pOwnerNode.fToolsOver.style.left = 0;
							pOwnerNode.fSliderHeight = scImageMgr.xGetEltHeight(vCurrentImage);
						} else {
							pOwnerNode.fToolsOver.style.left = pOwnerNode.fSliderWidth / 2 - vCurrentImage.fWidth / 2  + "px";
							pOwnerNode.fSliderHeight = pOwnerNode.fOrigHeight;
						}
						pOwnerNode.style.height = pOwnerNode.fSliderHeight + "px";
						pOwnerNode.fToolsOver.style.height = pOwnerNode.fSliderHeight + "px";
						pOwnerNode.fToolsOver.style.width = vCurrentImage.fWidth + "px";
						for (var k=0; k<pOwnerNode.fImgs.length; k++) {
							var vImg = pOwnerNode.fImgs[k];
							if (vImg.src) {
								if (pOwnerNode.fSliderWidth < vImg.fOrigWidth) {
									vImg.style.left = pOwnerNode.fCurrentImageIndex !== k ? (pOwnerNode.fSliderWidth + "px") : 0;
									vImg.fWidth = pOwnerNode.fSliderWidth;
									vImg.style.width = vImg.fWidth + "px";
									vImg.style.height = "auto";
								} else {
									vImg.fWidth = scImageMgr.xGetEltWidth(vImg);
									vImg.style.left = pOwnerNode.fCurrentImageIndex !== k ? (pOwnerNode.fSliderWidth + "px") : (pOwnerNode.fSliderWidth / 2 -  vCurrentImage.fWidth / 2 + "px");
									vImg.style.height = pOwnerNode.fSliderHeight + "px";
...
scImageMgr.xRunSlider = function(pSlider) {
	...
	// On charge l'image suivante si elle est pas chargée
	if (pSlider.fImgs[pSlider.fCurrentImageIndex + 1] && !pSlider.fImgs[pSlider.fCurrentImageIndex + 1].src) pSlider.fImgs[pSlider.fCurrentImageIndex + 1].src = pSlider.fImgs[pSlider.fCurrentImageIndex + 1].getAttribute("data-src");
	var vCurrentImageWidth = pSlider.fImgs[pSlider.fCurrentImageIndex].fWidth;
	...
	pSlider.fSliderTimer = setTimeout(function() {
		...
		if (pSlider.fImgs[pSlider.fPrevImageIndex]) pSlider.fImgs[pSlider.fPrevImageIndex].style.left = -pSlider.fImgs[pSlider.fPrevImageIndex].fWidth  + "px";
