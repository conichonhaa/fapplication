...
if ($fap->isAdmin('diff')) {	
			...
			if ($_POST['message']!='') {
				...
				foreach ($fap->getListUsers() as $usr => $u) {
					if ($_POST['choix']=='all' || isset($_POST['pick'.$usr])) {
						...
						if ($info['tel']=='' or !$info['notif_ifttt']) continue;
						...
						$string .= "<script>sendMessageIfttt(".$usr.");</script>";
					}
				}
	...
	$string .= "<div class='cadre' >";
	$string .= "<div class='divEntete' >";
	$string .= "Envoyer une notification IFTTT";
	$string .= "</div>";
	...
	foreach ($t_users as $usr => $v) {
		...
		if ($userkey!='') {
			$check = ($info['notif_ifttt']?'checked':'disabled');
			...
			if ($info['notif_ifttt']) $nb++;
		} else {
			if ($sansmail!='') {
				$sansmail .= ", ";
				$verbe = " n'ont pas de clef IFTTT";
			}
			$sansmail .= $v['nom'];
		}
	}
